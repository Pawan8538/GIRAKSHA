...
============================================================ */
const insertSensorReading = async (sensor_id, value, status = 'ok') => {
  ...
  // Update the parent sensor with latest reading
  await query(
    `UPDATE sensors
     SET current_value = $2,
...
/* ============================================================
   MESSAGING
============================================================ */
const findConversation = (user1_id, user2_id) =>
  query(
    `SELECT * FROM conversations
     WHERE (gov_user_id = $1 AND site_admin_id = $2)
        OR (gov_user_id = $1 AND super_admin_id = $2)
        OR (site_admin_id = $1 AND super_admin_id = $2)
        OR (gov_user_id = $2 AND site_admin_id = $1)
        OR (gov_user_id = $2 AND super_admin_id = $1)
        OR (site_admin_id = $2 AND super_admin_id = $1)`,
    [user1_id, user2_id]
  );

const createConversation = (gov_user_id, site_admin_id, super_admin_id, created_by) =>
  query(
    `INSERT INTO conversations (gov_user_id, site_admin_id, super_admin_id, created_by)
     VALUES ($1, $2, $3, $4)
     RETURNING *`,
    [gov_user_id, site_admin_id, super_admin_id, created_by]
  );
...
const getConversationsForUser = (user_id) =>
  query(
    `SELECT c.*,
     ...
     LEFT JOIN users sup ON sup.id = c.super_admin_id
     WHERE c.gov_user_id = $1 OR c.site_admin_id = $1 OR c.super_admin_id = $1
...
module.exports = {
  ...
  getAdvisories,
  getAllAlerts: () => query(`SELECT * FROM alerts ORDER BY created_at DESC`),
  ...
  getUsersBySlope: (slope_id) =>
    query(`SELECT * FROM users WHERE slope_id = $1`, [slope_id]),

  deleteUser: (id) =>
    query(`DELETE FROM users WHERE id = $1 RETURNING *`, [id]),

  // SENSOR SCHEMA FIX
  ensureSensorSchema: async () => {
    console.log('Verifying sensor schema...');
    try {
      await query(`
        ALTER TABLE sensors 
      ...
      // Ensure existing rows have is_active set
      await query(`UPDATE sensors SET is_active = TRUE WHERE is_active IS NULL`);

      console.log('Sensor schema verified.');
    } catch (err) {
      console.error('Sensor schema verification failed:', err);
    }
  }
};
