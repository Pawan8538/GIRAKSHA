...
const createAlertController = async (req, res, next) => {
  try {
    ...
    if (io) {
      io.emit('newAlert', {
        id: newAlert.id,
        slope_id: newAlert.slope_id,
        alert_type: newAlert.alert_type,
        message: newAlert.message,
        severity: newAlert.severity,
        created_at: newAlert.created_at,
        type: newAlert.alert_type === 'sos' ? 'SOS' :
          newAlert.alert_type === 'i_am_safe' ? 'SAFE' :
            newAlert.alert_type === 'sensor' ? 'SENSOR' : 'SYSTEM',
        title: newAlert.alert_type === 'sos' ? '🆘 SOS Emergency' :
          newAlert.alert_type === 'i_am_safe' ? '✅ I Am Safe' :
            newAlert.alert_type === 'sensor' ? '⚠️ Sensor Alert' : 'System Alert'
      });
      console.log('🔔 Alert broadcasted via Socket.IO:', newAlert.id);
    }
  ...
  try {
    ...
    const combined = [
      ...alertsRes.rows.map(a => ({
        id: `alert-${a.id}`,
        type: a.alert_type === 'sos' ? 'SOS' :
          a.alert_type === 'i_am_safe' ? 'SAFE' :
            a.alert_type === 'safe' ? 'SAFE' :
              a.alert_type === 'sensor' ? 'SENSOR' : 'SYSTEM',
        title: a.alert_type === 'sos' ? '🆘 SOS Emergency' :
          a.alert_type === 'i_am_safe' ? '✅ I Am Safe' :
            a.alert_type === 'safe' ? '✅ I Am Safe' :
              a.alert_type === 'sensor' ? '⚠️ Sensor Alert' : 'System Alert',
        message: a.message,
        severity: a.alert_type === 'i_am_safe' || a.alert_type === 'safe' ? 'low' : a.severity,
