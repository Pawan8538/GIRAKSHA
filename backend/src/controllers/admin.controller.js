...
const getWorkerStatus = async (req, res, next) => {
  try {
    const onlineUsersMap = req.app.get('onlineUsers') || new Map();
    const onlineUserIds = Array.from(onlineUsersMap.keys());

    // If request query has 'role', we could filter further, 
    // but the requirement is "under workers only for site admin".
    // We will return the list of online user IDs and their timestamp (simulated or real).
    // Start simple: just the list of online IDs + status 'online'.

    const data = onlineUserIds.map(id => ({
      userId: id,
      status: 'online',
