        ...
        <>
            ...
            >
                ...
                {points.map((point, idx) => (
                    point.manual ? (
                        // Manual prediction marker - distinct blue diamond
                        <Marker
                            key={point.id || idx}
                        ...
                        // Automated sensor data marker - circle
                        <CircleMarker
                            key={point.id || idx}
                            ...
                            <Popup>
                                <div className="p-2 min-w-[200px]">
                                    ...
                                    <div className="space-y-1 text-sm">
                                        ...
                                        {point.mineProximity !== undefined && (
                                            <div className="flex justify-between">
                                                <span className="text-gray-600">Mine Proximity:</span>
                                                <span className="font-semibold">{(point.mineProximity * 100).toFixed(0)}%</span>
                                            </div>
                                        )}
                                        {point.sensorInfluence !== undefined && (
                                            <div className="flex justify-between">
                                                <span className="text-gray-600">Sensor Influence:</span>
                                                <span className="font-semibold">{(point.sensorInfluence * 100).toFixed(0)}%</span>
                                            </div>
                                        )}
                                    </div>
                                    <p className="text-xs text-gray-500 mt-2 border-t pt-1">Live data from fusion engine</p>
                                </div>
                            </Popup>
                        </CircleMarker>
                    )
