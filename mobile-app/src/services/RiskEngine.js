...
export const fetchWeatherData = async () => {
    ...
    try {
        // Use Open-Meteo (Free, No API Key Required, More Reliable)
        const url = `https://api.open-meteo.com/v1/forecast?latitude=${LAT}&longitude=${LON}&current=temperature_2m,relative_humidity_2m,precipitation,rain,wind_speed_10m&hourly=precipitation&daily=precipitation_sum&timezone=auto`;
        const response = await fetch(url);
        const data = await response.json();

        if (!data || !data.current) throw new Error('Invalid response from weather API');
        ...
        const rain72h = daily.precipitation_sum.slice(0, 3).reduce((a, b) => a + (b || 0), 0);
        const maxRainIntensity = Math.max(...hourly.precipitation.slice(0, 24).map(p => p || 0));

        // Determine weather condition from precipitation
        let weatherCondition = 'Clear';
        if (current.precipitation > 10) weatherCondition = 'Thunderstorm';
        else if (current.precipitation > 2) weatherCondition = 'Rain';
        else if (current.precipitation > 0) weatherCondition = 'Drizzle';
        else if (current.precipitation === 0) weatherCondition = 'Clouds';
